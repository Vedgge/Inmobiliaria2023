document.addEventListener('DOMContentLoaded', function () {
  ...
  for (const URLPatron in URLMapeadoElemento) {
    if (URLActual.includes(URLPatron)) {
      const selector = URLMapeadoElemento[URLPatron];
      const navbarUlElement = document.querySelector(selector);
      if (navbarUlElement) {
        navbarUlElement.classList.add("active");
      }
    }
  }
